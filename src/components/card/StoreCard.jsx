...
const StoreCard = ({ store = {}, onRun }) => {
  ...
  const { data: followingData, isLoading: isFollowingLoading } = useQuery({
    ...
    queryFn: () => {
      try {
        const { _id } = getToken()
        return checkFollowingStore(_id, store._id)
      } catch (error) {
        console.log(error)
  ...
  useEffect(() => {
    if (!isLoading && isMounted.current) {
      const newStore = {
        ...
        numberOfFollowers: followersData?.count || 0,
        isFollowing: followingData?.success || false
      }
        ...
        <div className='col-md-4 d-flex align-items-center justify-content-center'>
          {isLoading ? (
            ...
            <Link to={`/store/${storeValue._id}`}>
              <img
                loading='lazy'
                src={`${IMG}/stores/${storeValue.avatar}`}
                className='card-img'
                alt={storeValue.name}
              />
            </Link>
              ...
              <>
                ...
                >
                  ...
                  <div className='d-flex gap-2 align-items-center'>
                    <StoreFollowLabel level={storeValue.level || 0} />
                    <FollowStoreButton
