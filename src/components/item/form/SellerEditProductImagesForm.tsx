...
interface SellerEditProductImagesFormProps {
  product?: any
  storeId?: string
  onRun?: () => void
}

const SellerEditProductImagesForm = ({
  product = {},
  storeId = '',
  onRun
}: SellerEditProductImagesFormProps) => {
  return (
    <div className='position-relative'>
      <form className='row' onSubmit={(e) => e.preventDefault()}>
        ...
        <div className='col-12 px-4 my-3 d-flex flex-wrap justify-content-start align-items-end'>
          ...
          {product.listImages?.map((img: string, index: number) => {
            if (index === 0) return
            return (
