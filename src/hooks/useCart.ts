...
export const useAddToCart = () => {
	...
	return useMutation({
		mutationFn: ({ userId, cartItem }: { userId: string; cartItem: any }) => addToCart(userId, cartItem),
...
export const useUpdateCartItem = () => {
	const queryClient = useQueryClient()
	return useMutation({
		mutationFn: ({ userId, cartId, quantity }: { userId: string; cartId: string; quantity: number }) => updateCartItem(userId, quantity, cartId),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: cartKeys.lists() })
		}
	})
...
export const useRemoveCartItem = () => {
	const queryClient = useQueryClient()
	return useMutation({
		mutationFn: ({ userId, cartId }: { userId: string; cartId: string }) => deleteFromCart(userId, cartId),
