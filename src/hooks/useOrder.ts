...
export const orderKeys = {
	...
	list: (userId: string, filters: FiltersType) => [...orderKeys.lists(), userId, { filters }],
	...
	detail: (orderId: string, userId: string) => [...orderKeys.details(), orderId, userId],
	...
	itemsByOrder: (orderId: string, userId: string) => [...orderKeys.items(), orderId, userId],
...
export const useOrderByStore = (userId: string, orderId: string, storeId: string) => {
	return useQuery({
...
export const useOrderItemsByStore = (userId: string, orderId: string, storeId: string) => {
	return useQuery({
...
export const useOrdersByStore = (userId: string, filters: FiltersType, storeId: string) => {
	return useQuery({
...
export const useReturnsByStore = (userId: string, filters: FiltersType, storeId: string) => {
	return useQuery({
...
export const useOrdersForAdmin = (filters: FiltersType) => {
	return useQuery({
		...
		queryFn: () => listOrdersForAdmin(filters),
		enabled: true
	})
}

// Order count hook
export const useOrderCount = (status: string, userId: string, storeId: string) => {
	return useQuery({
...
export const useCreateOrder = () => {
	...
	return useMutation({
		mutationFn: ({ userId, cartId, order }: { userId: string; cartId: string; order: any }) => createOrder(userId, cartId, order),
		onSuccess: (data: any, variables: { userId: string; cartId: string; order: any }) => {
...
export const useCreateReturnRequest = () => {
	const queryClient = useQueryClient()
	return useMutation({
		mutationFn: ({ userId, orderId, reason }: { userId: string; orderId: string; reason: string }) => createReturnRequest(userId, orderId, reason),
		onSuccess: (data: any, variables: { userId: string; orderId: string; reason: string }) => {
			queryClient.invalidateQueries({ queryKey: orderKeys.detail(variables.orderId, variables.userId) })
			queryClient.invalidateQueries({ queryKey: orderKeys.lists() })
		}
	})
}
...
export const useUserCancelOrder = () => {
	const queryClient = useQueryClient()
	return useMutation({
		mutationFn: ({ userId, status, orderId }: { userId: string; status: string; orderId: string }) => userCancelOrder(userId, status, orderId),
		onSuccess: (data: any, variables: { userId: string; status: string; orderId: string }) => {
			queryClient.invalidateQueries({ queryKey: orderKeys.detail(variables.orderId, variables.userId) })
...
export const useSellerUpdateOrderStatus = () => {
	...
	return useMutation({
		...
		onSuccess: (data: any, variables: { userId: string; status: string; orderId: string; storeId: string }) => {
			queryClient.invalidateQueries({ queryKey: orderKeys.storeDetail(variables.orderId, variables.storeId, variables.userId) })
			queryClient.invalidateQueries({ queryKey: orderKeys.storeList(variables.storeId, variables.userId, {}) })
		}
	})
}
...
export const useSellerUpdateReturnStatus = () => {
	...
	return useMutation({
		...
		onSuccess: (data: any, variables: { userId: string; status: string; orderId: string; storeId: string }) => {
			queryClient.invalidateQueries({ queryKey: orderKeys.returnList(variables.storeId, variables.userId, {}) })
			queryClient.invalidateQueries({ queryKey: orderKeys.storeDetail(variables.orderId, variables.storeId, variables.userId) })
		}
	})
}
